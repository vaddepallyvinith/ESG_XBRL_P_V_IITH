...
def banner():
    ...
    ╔════════════════════════════════════════════════════════════════╗
    ║                                                                ║
    ║   BRSR-ESRS Ontology-Guided Semantic Mapping Framework          ║
    ║   ────────────────────────────────────────────────────          ║
    ║   Phase 1 ─ PDF Parsing & Structured JSON (ESRS + BRSR)        ║
    ║   Phase 2 ─ Ontology Construction (BRSR + ESRS)               ║
...
def run_phase1(config: dict, company: str = None):
    ...
    logger.info("═══ PHASE 1: PDF Parsing & Structured JSON (ESRS) ═══")
    t0 = time.time()

    from parser.pdf_parser import parse_raw_directory
    from parser.section_segmenter import SectionSegmenter, GRISegmenter, ESRSSegmenter
    ...
    logger.info(f"Parsed {len(documents)} documents from {raw_dir}")

    # Segment into hierarchical trees
    brsr_segmenter = SectionSegmenter()
    gri_segmenter = GRISegmenter()
    esrs_segmenter = ESRSSegmenter()
    trees = []
    
    for doc in documents:
        fn_upper = doc.file_name.upper()
        is_esrs = "ESRS" in fn_upper or "esrs" in str(raw_dir).lower()
        is_gri = "GRI" in fn_upper
        logger.info(f"📄 Segmenting: {doc.file_name} (is_esrs={is_esrs}, is_gri={is_gri})")
        if is_esrs:
            tree = esrs_segmenter.segment(doc)
        elif is_gri:
