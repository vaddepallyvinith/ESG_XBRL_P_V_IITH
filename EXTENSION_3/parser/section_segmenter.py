...
# ─── ESRS Data Structures & Segmenter ───────────────────────────────────────

@dataclass
class ESRSRequirementNode:
    """A requirement within an ESRS disclosure."""
    type: str = ""
    text: str = ""
    content_blocks: List[str] = field(default_factory=list)
    tables: List[Dict[str, Any]] = field(default_factory=list)
...
@dataclass
class ESRSDisclosureNode:
    """An individual ESRS Disclosure Requirement."""
    id: str = ""
    label: str = ""
    text: str = ""
    requirements: List[ESRSRequirementNode] = field(default_factory=list)
    content_blocks: List[str] = field(default_factory=list)
    tables: List[Dict[str, Any]] = field(default_factory=list)
    page_start: int = 0
    page_end: int = 0
...
@dataclass
class ESRSStandardNode:
    """An ESRS Standard (e.g. ESRS E1 Climate Change)."""
    standard_id: str = ""
    title: str = ""
    disclosures: List[ESRSDisclosureNode] = field(default_factory=list)
    content_blocks: List[str] = field(default_factory=list)
    tables: List[Dict[str, Any]] = field(default_factory=list)
    page_start: int = 0
    page_end: int = 0
...
@dataclass
class ESRSDocumentTree:
    """Complete hierarchical structure of an ESRS standard document."""
    source_file: str = ""
    total_pages: int = 0
    standards: List[ESRSStandardNode] = field(default_factory=list)
    content_blocks: List[str] = field(default_factory=list)
    tables: List[Dict[str, Any]] = field(default_factory=list)

class ESRSSegmenter:
    """Segment a parsed ESRS document into hierarchical sections."""

    def __init__(self):
        self.heading_detector = HeadingDetector()

    def segment(self, document: DocumentContent) -> ESRSDocumentTree:
        """Build the ESRS document tree from parsed content."""
        tree = ESRSDocumentTree(
            source_file=document.file_name,
            total_pages=document.total_pages,
        )
        ...
        # Infer standard title from filename if not explicitly hit
        default_std_id = "ESRS Standard"
        fn = document.file_name.upper()
        if "E1" in fn: default_std_id = "ESRS E1"
        elif "E2" in fn: default_std_id = "ESRS E2"
        elif "E3" in fn: default_std_id = "ESRS E3"
        elif "E4" in fn: default_std_id = "ESRS E4"
        elif "E5" in fn: default_std_id = "ESRS E5"
        elif "201" in fn: default_std_id = "ESRS 1"
        elif "202" in fn: default_std_id = "ESRS 2"

        current_standard = ESRSStandardNode(
            standard_id=default_std_id,
            title=f"{default_std_id} Standard",
            page_start=1,
            page_end=document.total_pages,
        )
        tree.standards.append(current_standard)

        current_disclosure: Optional[ESRSDisclosureNode] = None
        current_req: Optional[ESRSRequirementNode] = None
        ...
        for block_info in classified_blocks:
            ...
            if heading and heading.is_heading:
                if heading.heading_type == "esrs_standard":
                    current_standard = ESRSStandardNode(
                        standard_id=heading.section_label,
                        title=heading.text,
                        page_start=page_num,
                ...
                elif heading.heading_type == "esrs_disclosure":
                    current_disclosure = ESRSDisclosureNode(
                        id=heading.section_label,
                        label=heading.text[:120],
                        text=heading.text,
                        page_start=page_num,
                ...
                elif heading.heading_type == "esrs_subsection":
                    current_req = ESRSRequirementNode(
                        type=heading.section_label,
                        text=heading.text,
                        page_start=page_num,
                        page_end=page_num,
        ...
        logger.info(
            f"Segmented ESRS Document {document.file_name}: "
            f"{len(tree.standards)} standards, "
            f"{sum(len(s.disclosures) for s in tree.standards)} disclosures"
        )
        return tree
    ...
    def _classify_all_blocks(self, document: DocumentContent) -> List[Dict]:
        """Classify every block in the document with heading info using ESRS mode."""
        result = []
        for page in document.pages:
            for block in page.blocks:
                heading = None
                if block.block_type == "text":
                    heading = self.heading_detector.classify_block(block, is_esrs=True)
