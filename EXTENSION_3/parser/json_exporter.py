...
from parser.section_segmenter import ESRSDocumentTree, ESRSStandardNode, ESRSDisclosureNode, ESRSRequirementNode


def esrs_requirement_to_dict(req: ESRSRequirementNode) -> Dict[str, Any]:
    return {
        "type": req.type,
        "text": req.text,
        "content": req.content_blocks,
...
def esrs_disclosure_to_dict(d: ESRSDisclosureNode) -> Dict[str, Any]:
    return {
        "id": d.id,
        "label": d.label,
        "text": d.text,
        "requirements": [esrs_requirement_to_dict(r) for r in d.requirements],
        "content": d.content_blocks,
        "tables": d.tables,
        "page_start": d.page_start,
        "page_end": d.page_end,
...
def esrs_standard_to_dict(s: ESRSStandardNode) -> Dict[str, Any]:
    return {
        "standard_id": s.standard_id,
        "title": s.title,
        "page_start": s.page_start,
        ...
        "content": s.content_blocks,
        "tables": s.tables,
        "disclosures": [esrs_disclosure_to_dict(d) for d in s.disclosures],
    }

def esrs_document_tree_to_dict(tree: ESRSDocumentTree) -> Dict[str, Any]:
    return {
        "framework": "ESRS",
        "source_file": tree.source_file,
        "total_pages": tree.total_pages,
        "content": tree.content_blocks,
        "tables": tree.tables,
        "standards": [esrs_standard_to_dict(s) for s in tree.standards],
    }


def export_structured_json(tree: Any, output_path: str):
    """Export a single document tree to JSON."""
    if isinstance(tree, ESRSDocumentTree):
        data = esrs_document_tree_to_dict(tree)
...
def export_batch(trees: List[Any], output_dir: str):
    ...
    for tree in trees:
        if isinstance(tree, ESRSDocumentTree):
            filename = f"ESRS_{tree.source_file.replace('.pdf', '')}.json"
