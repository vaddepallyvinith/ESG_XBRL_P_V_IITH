...
# ESRS structural patterns
ESRS_PATTERNS = {
    "standard_title": re.compile(r"^(ESRS\s+(?:[12]|E[1-5]))(?:\s+.*|\b)", re.IGNORECASE),
    "disclosure": re.compile(r"^(?:Disclosure\s+[Rr]equirement\s+(?:related\s+to\s+ESRS\s+2\s+)?([A-Z0-9_\-]+(?:\s*–\s*|\s*-\s*)?[^\n]*)|(E[1-5]-\d+|GOV-\d+|SBM-\d+|IRO-\d+|MDR-[PAMT]|BP-\d+)\b.*)", re.IGNORECASE),
    "subsection": re.compile(r"^(Objective|Interactions?\s+with\s+other\s+ESRS|Application\s+Requirements|Minimum\s+disclosure\s+requirement)\b", re.IGNORECASE),
}
...
class HeadingDetector:
    """Detect and classify headings in BRSR, GRI, and ESRS documents."""

    def __init__(self, font_thresholds=None):
        self.thresholds = font_thresholds or {"h1": 16.0, "h2": 13.0, "h3": 11.0, "body": 9.0}

    def classify_block(self, block: TextBlock, is_gri: bool = False, is_esrs: bool = False) -> HeadingInfo:
        ...
        if is_esrs:
            pattern_result = self._match_esrs_pattern(text, block.page_num, font_level)
        elif is_gri:
    ...
    def _match_esrs_pattern(self, text: str, page_num: int, font_level: int) -> Optional[HeadingInfo]:
        """Check for ESRS-specific structural patterns."""
        if ESRS_PATTERNS["standard_title"].match(text):
            m = ESRS_PATTERNS["standard_title"].match(text)
            label = m.group(1).upper()
            return HeadingInfo(
                level=1,
                heading_type="esrs_standard",
                section_label=label,
                text=text,
                page_num=page_num,
                is_heading=True,
            )
        if ESRS_PATTERNS["disclosure"].match(text):
            return HeadingInfo(
                level=2,
                heading_type="esrs_disclosure",
                section_label=text[:100],
                text=text,
                page_num=page_num,
                is_heading=True,
            )
        if ESRS_PATTERNS["subsection"].match(text):
            return HeadingInfo(
                level=3,
                heading_type="esrs_subsection",
                section_label=text[:60],
                text=text,
                page_num=page_num,
                is_heading=True,
            )
